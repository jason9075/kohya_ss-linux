...
def generate_step_logs(
    args: argparse.Namespace, current_loss, avr_loss, lr_scheduler, image_loss
):
    logs = {
...
def train(args):
    ...
    """ first_parameter = next(text_encoder.parameters()) """
    """ dummy_text_input = torch.zeros((1, 77), device="cpu", dtype=torch.int64) """
    ...
    """     verbose=False, """
    """ ) """
    """ print("save text_encoder.onnx") """

    """ device = "meta" """
    """ dummy_unet_image_input = torch.zeros( """
    """     (1, 4, 64, 64), device=device, dtype=torch.float32 """
    """ ) """
    """ dummy_unet_time_embedding_input = torch.zeros( """
    """     (1), device=device, dtype=torch.float32 """
    """ ) """
    """ dummy_unet_hidden_state_input = torch.zeros( """
    """     (1, 1, 768), device=device, dtype=torch.float32 """
    """ ) """
    """ unet.to(device) """
    """ model_graph = draw_graph( """
    """     unet, """
    """     input_data=( """
    """         dummy_unet_image_input, """
    """         dummy_unet_time_embedding_input, """
    """         dummy_unet_hidden_state_input, """
    """     ), """
    """     save_graph=True, """
    """     directory="./figures/", """
    """     device=device, """
    ...
    lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, rule)
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            current_image_loss = image_loss.detach().item()
            ...
            logs = {
                "loss": avr_loss,
                "image_loss": current_image_loss,
                "text_lr": lr_scheduler.get_last_lr()[0],
                "unet_lr": lr_scheduler.get_last_lr()[1],
            }
            progress_bar.set_postfix(**logs)

            if args.logging_dir is not None:
                logs = generate_step_logs(
                    args, current_loss, avr_loss, lr_scheduler, current_image_loss
                )
